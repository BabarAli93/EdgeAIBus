...
class DatacenterGeneration:
    def __init__(self, config: Dict[str, Any]):
        ...
        #random.seed(seed)
        self.num_clusters = config['nums']['num_clusters']
        self.num_hosts = config['nums']['num_hosts'] ## Number of hosts or nodes
        ...
        # make sure each node and container has CPU and RAM
        for _, key in self.container_conf.items():
            assert len(key) == self.num_resources, 'each container is not given cpu and ram'
        
        for _, key in self.hosts_cap_rng.items():
        ...
        self.kube = False
        if self.type_env.startswith("sim"):
    ...
    def randomDeployment(self):
        ...
        # now Make GKE deployment 
            ...
            raise Exception(f"Please provide simulation or Kubernetes mode in type_env")
    ...
    def hosts_resources_capacities_sim(self, hosts_cap_rng):
        ...
        for index, key in hosts_cap_rng.items():
            nodes_ram_cap = key['ram']['cap']
            nodes_ram[int(index)] = nodes_ram_cap

        nodes_cpu = np.zeros((self.num_hosts, 1))
        for index, key in hosts_cap_rng.items():
        ...
        hosts_resources_cap = np.concatenate((nodes_ids, nodes_cpu, nodes_ram),
                                                    axis=1)
        return hosts_resources_cap.astype(int)
    ...
    def hosts_resources_allocatable_sim(self, hosts_cap_rng):
        nodes_ids = np.arange(self.num_hosts).reshape(self.num_hosts, 1)
        nodes_ram = np.zeros((self.num_hosts, 1))
        for index, key in hosts_cap_rng.items():
            nodes_ram_cap = key['ram']['alloc']
            nodes_ram[int(index)] = nodes_ram_cap

        nodes_cpu = np.zeros((self.num_hosts, 1))
        for index, key in hosts_cap_rng.items():
            nodes_cpu_cap = key['cpu']['alloc']
            nodes_cpu[int(index)] = nodes_cpu_cap

        hosts_resources_alloc = np.concatenate((nodes_ids, nodes_cpu, nodes_ram),
                                                    axis=1)
    ...
    def hosts_resources_remaining_init(self, hosts_cap_rng):
        ## this function calculates the resources based on the profiling. These will be appended to further requested ones
        nodes_ids = np.arange(self.num_hosts).reshape(self.num_hosts, 1)
        nodes_ram = np.zeros((self.num_hosts, 1))
        for index, key in hosts_cap_rng.items():
            nodes_ram_cap = key['ram']['remain']
            nodes_ram[int(index)] = nodes_ram_cap

        nodes_cpu = np.zeros((self.num_hosts, 1))
        for index, key in hosts_cap_rng.items():
            nodes_cpu_cap = key['cpu']['remain']
            nodes_cpu[int(index)] = nodes_cpu_cap

        hosts_resources_remain = np.concatenate((nodes_ids, nodes_cpu, nodes_ram),
                                                    axis=1)

        return hosts_resources_remain.astype(int)
