...
class Mapper:
    ...
    # def _generate_tuple_to_int_mapping(self):
    #     # get the tuples
    #     action_integers = []
    #     tuples = self._generate_mapping_dict()
    #     for tuple_value in tuples:
    #         host_id, model_version = tuple_value
    #         action_int = host_id * 2 + model_version
    #         action_integers.append(action_int)

    #     return np.array(action_integers)
    
    def _generate_tuple_to_int_mapping(self):
    # get the tuples
        tuple_to_int_mapping = {}
        tuples = self._generate_mapping_dict()
        for tuple_value in tuples:
            tuple_value = tuple(tuple_value)
            host_id, model_version = tuple_value
            action_int = host_id * 2 + model_version
            tuple_to_int_mapping[tuple_value] = action_int

        return tuple_to_int_mapping
